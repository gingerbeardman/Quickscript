    ...
    if (lastVersion < 6) {
      Object.keys(localStorage).forEach(function (key) {
        try {
          ...
          if (so.id) {
            so.autorun = true;
            localStorage[key] = JSON.stringify(so);
          }
        } catch(e) {}
      });
    }
...
function getMatchingScripts(url) {
  ...
  var scriptItem;
  Object.keys(localStorage).forEach(function (key) {
    scriptItem = JSON.parse(localStorage[key]);
...
function rebuildScriptMenu() {
  ...
  Object.keys(onDemandScripts).forEach(function (key) {
    if (onDemandScripts[key].patterns.some(isGlobalPattern)) {
      globalScripts.push(onDemandScripts[key]);
    } else if (onDemandScripts[key].patterns.some(matchesCurrentUrl)) {
      localScripts.push(onDemandScripts[key]);
    }
  });
...
var jQueryUrl = ext.baseURI + 'jquery-3.1.1.min.js';
var autorunScripts = {};
var onDemandScripts = {};
